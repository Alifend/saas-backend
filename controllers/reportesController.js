...
exports.bestSeller = async (req, res) => {
  ...
  const query =
    "SELECT p.nombre, SUM(dv.cantidad) as contador FROM DetalleVenta dv INNER JOIN Products p ON dv.ProductId = p.id GROUP BY ProductId ORDER BY contador DESC ;";
  const result = await model.sequelize.query(query);
  res.send(result[0]);
};
...
exports.bestBuyer = async (req, res) => {
  ...
  const query =
    "SELECT u.nombre, SUM(v.Total) as contador FROM Venta v  INNER JOIN Users u ON v.UserId = u.id GROUP BY UserId ORDER BY contador DESC ;";
